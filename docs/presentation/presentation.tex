...
\begin{frame} 
\frametitle{\insertsection}\pause  
\begin{itemize} 
	\item MyMediaLite\pause
	\item PREA (Personalized Recommandation Algorithms Toolkit)\pause
	\item Apache Mahout\pause
	\item Duine Framework\pause
	\item Cofi\pause
	\item Lenskit %written in Java
\end{itemize}
\end{frame}
\section{Recsyslab}
\frame{
...
\begin{itemize}
    ...
    \item For education\pause
    \item For research\pause
...
\begin{equation}
	\onslide<2->{
    \text{sim}(i,j) = 
	 	\cos(\vec{i}, \vec{j})=\frac{\vec{i} \cdot \vec{j}}{||\vec{i}||_{2} ||\vec{j}||_{2}}}
\end{equation}
...
\begin{frame}
    \frametitle{Leave-one-out Protocol}\pause
    \begin{enumerate}
...
\begin{itemize}
    ...
    \item For education \visible<4->{$\checkmark$}
    \item For research \visible<5->{$\checkmark$}
    \item Open source license: GPLv3 \visible<6->{$\checkmark$}
\end{itemize}
\end{frame}
